...
export async function updateScheduleDays(
  ...
  const scheduleDays = await (
    await getCollection('scheduleDay')
  )
    .find({
  ...
  const aggregateResult = await db
    .collection<{ completedPartners: number }>('scheduleDay')
    .aggregate([
      {
        $match: {
          scheduleId: schedule._id,
          dayId: { $lt: schedule.completedDays },
        },
      },
  ...
  const partnersPerDay: number = Math.floor(
    incompletePartners.length / numDays,
  );
  ...
  for await (const [index, day] of scheduleDays.entries()) {
    ...
    const endIndex =
      numDistributedPartners +
      partnersPerDay +
      (index < remainderPartners ? 1 : 0);
...
async function createSchedule(dirtyMonth: Date): Promise<ScheduleModel> {
  ...
  await (
    ...
    range(numDaysInMonth).map((dayId) => {
...
export async function getOrCreateSchedule(
  ...
  const month = startOfMonth(dirtyMonth);
  return (
    (await (await getCollection('schedule')).findOne({ month })) ||
    createSchedule(month)
  );
}
...
export async function setSkippedDayStatus(
  scheduleId: ObjectId,
  dayId: number,
  isSkipped: boolean,
): Promise<ScheduleModel> {
  ...
  const { modifiedCount } = await (
    await getCollection('scheduleDay')
  ).updateOne({ scheduleId, dayId }, { $set: { isSkipped } });
  if (modifiedCount === 0) {
